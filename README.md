...
+  **通用处理方法：**
  1. 关闭杀毒软件，杀毒软件可能会阻止注册系统代理，访问端口号
  2. 关闭Windows网络防火墙(设置-搜索) ，可能会阻止访问端口号

+  **以一种访问权限不允许的方式做了一个访问套接字的尝试**
  1. 可能是端口被占用了，请尝试前往配置文件将8888端口号修改为别的号码(1000~65535)
  2. 关杀毒
+ **尝试执行未经授权的操作**
  1. 关杀毒
+ 以上都不行
  1. 请前往[这里](https://www.baidu.com/s?word=电脑购买)瞧瞧 

### 浏览器总是连接一段时间后，就在在控制台看不到弹幕了

+ 优先使用Chrome 和 Edge 浏览器，其他浏览器(例如360浏览器) 已知会存在该问题
